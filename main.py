...
def extract_filters(user_message: str, previous_filters: dict):
    ...
    **📌 قوانین پردازش:**
    - 🚨 اگر یکی از موارد max_price: حداکثر بودجه، district: منطقه موردنظر، bedrooms: تعداد اتاق‌خواب در پیام کاربر یا در اطلاعات قبلی کاربر موجود نیست مقدار "search_ready": false قرا بده و سؤالات پیشنهادی را که اطلاعاتش توسط کاربر داده نشده را بپرس.
    ...
    - 🚨 اگر کاربر درباره `disrict` یا `max_price` یا `bedrooms` چیزی نگفت یا عباراتی مانند "برام فرقی نداره"، "هرچند اتاق باشه اوکیه"، "مهم نیست چندتا اتاق داشته باشه" را گفت، مقدار `bedrooms` را `null` قرار بده و آن را در فیلتر لحاظ نکن.
...
def generate_ai_summary(properties, start_index=0):
    ...
    **📌 مقدار 'st_index' برای شماره‌گذاری املاک: {st_index}**
    ...
    **قوانین پاسخ:**
    - شماره گذاری برای معرفی املاک رو از 'st_index' به بعد شروع کن مثلا اگر مقدارش 4 هست از 4 تا 6 املاک رو موقع معرفی نامگذاری کن و به همین صورت عدد رو بالا ببر
    ...
    **در انتهای پیام حتماً این جمله را اضافه کن:**  
    **"برای مشاهده اطلاعات بیشتر درباره هر ملک، لطفاً عبارت 'پروژه شماره X را بیشتر توضیح بده' را بنویسید."**  
    **"اگر به املاک بیشتری نیاز دارید، بگویید: 'املاک بیشتری نشان بده'."**  
    """

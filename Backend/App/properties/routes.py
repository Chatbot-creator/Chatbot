...
async def get_filters():
    """
    دریافت لیست فیلترهای موجود برای املاک با استفاده از کش
    """
    cache_key = "property_filters"
    cached_filters = get_cached_data(cache_key)
    ...
    set_cached_data(cache_key, filters)
    return filters
...
@router.post("/filter", response_model=PropertyResponseWrapper)
async def filter_properties_route(
    ...
    """
    فیلتر کردن املاک با پارامترهای ورودی با استفاده از کش
    """
    # ایجاد کلید کش بر اساس پارامترهای فیلتر و مرتب‌سازی
    cache_key = f"filtered_properties_{hash(str(params)+str(sorting))}"
    ...
    properties, total_count = await filter_properties_db(db, params, sorting)
    
    if not properties:
        return {"properties": [], "total": 0}
    
    result = {
        "properties": [convert_to_original_format(prop) for prop in properties],
        "total": total_count
    }
